...
import {
    convertToGeneAGeneBRepresentation,
    parseOQLQuery,
...
export class StudyViewPageStore
    ...
    */
    ...
    @observable.ref private _structVarFilterSet = observable.map<
        string,
        StructVarFilterQuery[][]
    >();
    ...
    onCheckStructuralVariant(
        ...
        if (
            gene1hugoGeneSymbol !== undefined ||
            gene2HugoGeneSymbol !== undefined
        ) {
    ...
    @computed get selectedStructuralVariants(): StructVarGene1Gene2[] {
        return _(this.geneQueries)
            .filter(query => queryContainsStructVarAlteration(query))
            .map(query => oqlQueryToGene1Gene2Representation(query))
            .flatten()
            .compact() // Remove falsy elements.
            .uniqWith(
                (repr1, repr2) =>
                    repr1.gene1 === repr2.gene2 && repr1.gene2 === repr2.gene2
    ...
    addStructVarFilters(
        ...
        const queries: StructVarFilterQuery[][] = _.map(
    ...
    removeStructVarFilter(chartUniqueKey: string, toBeRemoved: string): void {
        ...
        const gene1Gene2Str = convertToGeneAGeneBRepresentation(oqlQuery)[0];
        const [
            gene1HugoSymbol,
            gene2HugoSymbol,
        ]: string[] = gene1Gene2Str.split('::');
        let structVarFilters: StructVarFilterQuery[][] =
        ...
        structVarFilters = _.reduce(
            ...
            (acc, next) => {
                const newGroup = next.filter(
                    structVarFilterQuery =>
                        structVarFilterQuery.gene1Query.hugoSymbol !==
                            gene1HugoSymbol &&
                        structVarFilterQuery.gene2Query.hugoSymbol !==
                            gene2HugoSymbol
                );
                if (newGroup.length > 0) {
                    acc.push(newGroup);
