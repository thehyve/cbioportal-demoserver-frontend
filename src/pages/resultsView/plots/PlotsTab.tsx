...
export default class PlotsTab extends React.Component<IPlotsTabProps,{}> {
    ...
    private initAxisMenuSelection(vertical:boolean):AxisMenuSelection {
        ...
        return observable({
            ...
            get selectedGeneOption() {
                const geneOptions = vertical ? self.vertGeneOptions : self.horzGeneOptions || [];
            ...
            get selectedGenesetOption() {
                const genesetOptions = (vertical ? self.vertGenesetOptions : self._genesetOptions.result) || [];
            ...
            get selectedTreatmentOption() {
                const treatmentOptions = (vertical ? self.vertTreatmentOptions : self._treatmentOptions.result) || [];
    ...
    @observable readonly _geneOptions = remoteData({
    ...
    @computed get horzGeneOptions() {
        let noGeneOption = undefined;
        if (this._geneOptions && this.vertSelection.dataType === AlterationTypeConstants.TREATMENT_RESPONSE) {
            noGeneOption = [{ value: NONE_SELECTED_OPTION_NUMERICAL_VALUE, label: NONE_SELECTED_OPTION_LABEL }];
        }
        return (noGeneOption || []).concat((this._geneOptions.result || []) as any[]);
    }
    ...
    @computed get vertGeneOptions() {
        ...
        if (this._geneOptions || this.horzSelection.dataType) {
        ...
        return (noGeneOption || []).concat((sameGeneOption || [])).concat((this._geneOptions.result || []) as any[]);
    }

    @observable readonly _genesetOptions = remoteData({
    ...
    @computed get horzGenesetOptions() {
        let noGenesetOption = undefined;
        if (this._genesetOptions && this.vertSelection.dataType === AlterationTypeConstants.TREATMENT_RESPONSE) {
            noGenesetOption = [{ value: NONE_SELECTED_OPTION_STRING_VALUE, label: NONE_SELECTED_OPTION_LABEL }];
        }
        return (noGenesetOption || []).concat((this._genesetOptions.result || []) as any[]);
    };
    ...
    @computed get vertGenesetOptions() {
        ...
        if (this._genesetOptions || this.horzSelection.dataType) {
            // when the data type on the horizontal axis is a treatment profile
            // add an option to vertical gene  option that allows to select `nothing`
            if (this.horzSelection.dataType === AlterationTypeConstants.TREATMENT_RESPONSE) {
                noGenesetOption = [{ value: NONE_SELECTED_OPTION_STRING_VALUE, label: NONE_SELECTED_OPTION_LABEL }];
        ...
        return (noGenesetOption || []).concat((sameGenesetOption || [])).concat((this._genesetOptions.result || []) as any[]);
    }

    @observable readonly _treatmentOptions = remoteData({
    ...
    @computed get horzTreatmentOptions() {
        let noTreatmentOption = undefined;
        if (this._treatmentOptions && this.vertSelection.dataType === AlterationTypeConstants.TREATMENT_RESPONSE) {
            noTreatmentOption = [{ value: NONE_SELECTED_OPTION_STRING_VALUE, label: NONE_SELECTED_OPTION_LABEL }];
        }
        return (noTreatmentOption || []).concat((this._treatmentOptions.result || []) as any[]);
    };
    ...
    @computed get vertTreatmentOptions() {
        ...
        return (noTreatmentOption || []).concat((sameTreatmentOption || [])).concat((this._treatmentOptions.result || []) as {value:string, label:string}[]);
    }

    showGeneSelectBox(dataType:string):boolean {
        return dataType !== GENESET_DATA_TYPE
    ...
    showTreatmentSelectBox(dataType:string):boolean {
        return dataType === AlterationTypeConstants.TREATMENT_RESPONSE;
    }
    ...
    readonly dataTypeOptions = remoteData<{value:string, label:string}[]>({
        ...
        invoke:()=>{
            ...
            if (this.props.store.molecularProfilesInStudies.result!.length && this._genesetOptions.result && this._genesetOptions.result!.length > 0) {
            // if (this.props.store.molecularProfilesInStudies.result!.length && this.horzGenesetOptions && this.horzGenesetOptions.length > 0) {
                    ...
                    {(axisSelection.dataType && this.showGeneSelectBox(axisSelection.dataType))
                        ...
                        <div style={{display:"flex", flexDirection:"row"}}>
                            <ReactSelect
                                ...
                                isLoading={this._geneOptions.isPending}
                                options={this.vertGeneOptions && this.horzGeneOptions? (vertical ? this.vertGeneOptions : this.horzGeneOptions): []}
                                clearable={false}
                                searchable={false}
                    ...
                    {(axisSelection.dataType && this.showGenesetSelectBox(axisSelection.dataType))
                        ...
                        <div style={{display:"flex", flexDirection:"row"}}>
                            <ReactSelect
                                ...
                                isLoading={this._genesetOptions.isPending}
                                options={this.vertGenesetOptions && this.horzGenesetOptions? (vertical ? this.vertGenesetOptions : this.horzGenesetOptions): []}
                                clearable={false}
                                searchable={false}
                    ...
                    {(axisSelection.dataType && this.showTreatmentSelectBox(axisSelection.dataType))
                        ...
                        <div style={{display:"flex", flexDirection:"row"}}>
                            <ReactSelect
                                ...
                                isLoading={this._treatmentOptions.isPending}
                                options={this.vertTreatmentOptions && this.horzTreatmentOptions? (vertical ? this.vertTreatmentOptions : this.horzTreatmentOptions): []}
                                clearable={false}
                                searchable={false}
