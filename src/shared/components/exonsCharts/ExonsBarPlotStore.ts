...
export default class ExonsBarPlotStore {
    ...
    get computedTotalWidth(): number {
        return this.transcript.totalWidth ?  this.transcript.totalWidth * this.divider : 0;
    }

    @computed
    get computedTotalRefGeneWidth(): number {
        return this.referenceGeneWidth ? this.referenceGeneWidth * this.divider : this.computedTotalWidth;
    }

    @computed
    get computedDeltaX(): number {
        return this.transcript.deltaX ? this.transcript.deltaX * this.divider : 0;
    }
    ...
    get computedExons(): ExonRangeExt[] {
        ...
        return (this.transcript.exons || []).map( exon => {
            if (prevExon) {
                ...
                exon.x = prevExon.x + (prevExon.width * this.divider);
            } else {
                exon.x = this.deltaXPos;
            }
            prevExon = exon;
            return exon;
        });
