...
import { ExonRangeExt, PfamDomainRangeExt, StructuralVariantExt, UtrType } from '../../model/Fusion';
...
describe('ExonsBarPlotStore', () => {
    describe('computedExons', () => {
        ...
        before(() => {
            store = new ExonsBarPlotStore({label: 'GENEX', isReference: true}, transcript);
            fusionStore = new ExonsBarPlotStore({label: 'GENEA-GENEB', isReference: false}, fusion, 3000);
            emptyStore = new ExonsBarPlotStore({label: 'EMPTY', isReference: false}, emptyTranscript);
        });
        ...
        it('should compute x positions for all exons when drawing ref gene transcript', () => {
            ...
            const expectedPfamXPos = transcript.pfamDomains.map(p => {
                const fivePrimeLength = transcript.fivePrimeLength ? transcript.fivePrimeLength : 0;
                const deltaX = transcript.deltaX ? transcript.deltaX : 0;
                return (p.pfamDomainStart * 3 * 0.1) + (fivePrimeLength * 0.1) + (deltaX * 0.1);
            });
            ...
            store.computedPfams.forEach((p: PfamDomainRangeExt, idx: number) => {
        ...
        it('should compute x positions for all exons when drawing fusion genes', () => {
            ...
            fusionStore.computedExons.forEach((e: ExonRangeExt, idx: number) => {
                assert.equal(e.x, expectedXPos[idx]);
            });
        });
    });
