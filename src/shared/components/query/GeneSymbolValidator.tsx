...
export default class GeneSymbolValidator extends QueryStoreComponent<{}, {}>
{
	render()
	{
		...
		if (this.store.isQueryLimitReached)
			return (
				<div className={styles.GeneSymbolValidator}>
					<div className={styles.invalidBubble} title="Please limit your queries to 200 genes or fewer.">
						<FontAwesome className={styles.icon} name='exclamation-circle'/>
						<span>Queries are limited to 200 genes. Please <a style={{color: '#ccc'}} href={`mailto:${AppConfig.serverConfig.skin_email_contact}`}>let us know</a> your use case(s) if you need to query more than 200 genes.</span>
					</div>
				</div>
			);
