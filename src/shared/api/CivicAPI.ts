...
export default class CivicAPI {
    ...
    return request.get('http://localhost:3000/api/genes/' + ids)
           ...
           .then((res) => {
                ...
                return result.map((record: CivicAPIGene) => ({
                    ...
                    url: 'http://localhost:3000/#/events/genes/'
                    + record.id + '/summary',
     ...
     */
     getVariant(id: number, name: string, geneId: number): Promise<ICivicVariantData> {
        return request.get('http://localhost:3000/api/variants/' + id)
               .then((res) => {
                   let result = res.body;
                   return {
                       id,
                       ...
                       url: 'http://localhost:3000/#/events/genes/' + geneId +
